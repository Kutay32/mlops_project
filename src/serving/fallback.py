        ...
        for i, (name, handler) in enumerate(self.fallback_handlers):
            ...
            try:
                ...
                result = PredictionResult(
                    ...
                    fallback_reason=(
                        FallbackReason.LOW_CONFIDENCE if is_fallback else None
                    ),
                    latency_ms=latency,
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "fallback_rate": (
                sum(
                    v
                    for k, v in self.fallback_counts.items()
                    if k != self.fallback_handlers[0][0]
                )
...
class FallbackMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_latency_ms": (
                np.mean([p["latency_ms"] for p in self.predictions])
                if self.predictions
                else 0
            ),

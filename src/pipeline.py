...
from src.data.feature_engineering import EmbeddingEncoder, HashingEncoder
    ...
    def build_pipeline(self):
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ...
                (
                    "hash",
                    HashingEncoder(n_features=8, col="PaymentMethod"),
                    hashing_features,
                ),
                # Apply EmbeddingEncoder to Contract
                (
                    "emb_contract",
                    EmbeddingEncoder(embedding_dim=4, col="Contract"),
                    ["Contract"],
                ),
                (
                    "emb_internet",
                    EmbeddingEncoder(embedding_dim=4, col="InternetService"),
                    ["InternetService"],
                ),
            ],
            remainder="drop",
        ...
        self.pipeline = Pipeline(
            [
                ...
                (
                    "classifier",
                    EnsembleModel(ensemble_type="voting", rebalancing_strategy="SMOTE"),
                ),
            ]
        )

...
class AdvancedEnsemble:
    ...
    def fit_weighted_voting(
        self, X_train, y_train, X_val, y_val, optimize_weights: bool = True
    ) -> dict:
        """
    ...
    def _optimize_weights(
        self, predictions: np.ndarray, y_true: np.ndarray, metric: str = "f1"
    ) -> np.ndarray:
        """
    ...
    def fit_blending(
        self, X_train, y_train, X_blend, y_blend, X_val, y_val, meta_learner=None
    ) -> dict:
        """
        ...
        if meta_learner is None:
            meta_learner = XGBClassifier(
                eval_metric="logloss", random_state=42, n_estimators=100, max_depth=3
            )

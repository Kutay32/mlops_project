...
class HyperparameterTuner:
    ...
    def tune_all_models(
        self, X, y, method: str = "grid", cv: int = 5, n_iter: int = 30
    ) -> pd.DataFrame:
        """
        ...
        for name in model_names:
            try:
                if method == "grid":
                    ...
                    result, estimator = self.random_search(
                        name, X, y, n_iter=n_iter, cv=cv
                    )
        ...
        return (
            pd.DataFrame(results).sort_values("best_score", ascending=False),
            best_estimators,
        )
